"""Default: camera-only dry run. --drive: bounded left-curve test."""
import argparse
import csv
import hashlib
import json
...
CORE_HASH = '00970c21b03f4a9caa2fcaabcd65096ae14f019dfcf50716fb93577498f96066'
...
def main():
    ...
    if args.self_test:
        self_test()
        return
    core = Path(__file__).with_name('s2_preview_core_context.py')
    if hashlib.sha256(core.read_bytes()).hexdigest() != CORE_HASH:
        raise RuntimeError('V2核心哈希不同，停止测试')
    import cv2
    from picamera2 import Picamera2
    from s2_preview_core_context import analyze
    out = Path('curve_context_results')/datetime.now().strftime('%Y%m%d_%H%M%S_%f')
    out.mkdir(parents=True)
    for name in ('v2_context_curve_test.py', 'bounded_state.py', 's2_preview_core_context.py'):
    ...
    try:
        if args.drive:
            confirm('左弯落地测试：驱动电机电池先断开。小车放在左弯起点的车道中央，车头沿当地车道方向，前方清空并准备断电。确认后输入YES：')
