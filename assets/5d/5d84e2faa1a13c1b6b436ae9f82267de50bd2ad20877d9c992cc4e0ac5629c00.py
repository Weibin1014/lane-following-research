...
CORE_HASH = '53d8c9a1bb137763067094725ae7e8d843429a7c91dbb4e8fe30207c3b325d7d'
...
def main():
    ...
    if args.self_test:
        self_test()
        return
    core = Path(__file__).with_name('s2_preview_core_edge.py')
    if hashlib.sha256(core.read_bytes()).hexdigest() != CORE_HASH:
        raise RuntimeError('V2核心哈希不同，停止测试')
    import cv2
    from picamera2 import Picamera2
    from s2_preview_core_edge import analyze
    out = Path('curve_edge_static_results')/datetime.now().strftime('%Y%m%d_%H%M%S_%f')
    out.mkdir(parents=True)
    for name in ('v2_edge_static.py', 'bounded_state.py', 's2_preview_core_edge.py'):
