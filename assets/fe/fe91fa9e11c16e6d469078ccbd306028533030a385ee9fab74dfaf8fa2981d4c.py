...
out = Path("steering_vision_results") / datetime.now().strftime("%Y%m%d_%H%M%S")
...
try:
    ...
    print("开始30秒视觉舵机测试；驱动电机必须断电；Ctrl+C结束。", flush=True)
    ...
    with (out / "frames.csv").open("w", newline="") as f:
        ...
        writer.writerow([
            ...
            "processing_ms", "track_status", "target_status", "offset", "steering_pwm"
        ])
