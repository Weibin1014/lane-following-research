"""Camera-only validation; --drive is disabled."""
import argparse
import csv
import hashlib
import json
...
CORE_HASH = 'a02017e638bb0b947e40d40c7ef7bd4724d74b03ed99e08708eb852d852a1696'
...
def main():
    ...
    if args.self_test:
        self_test()
        return
    core = Path(__file__).with_name('s2_preview_core_width.py')
    if hashlib.sha256(core.read_bytes()).hexdigest() != CORE_HASH:
        raise RuntimeError('V2核心哈希不同，停止测试')
    import cv2
    from picamera2 import Picamera2
    from s2_preview_core_width import analyze
    out = Path('curve_width_static_results')/datetime.now().strftime('%Y%m%d_%H%M%S_%f')
    out.mkdir(parents=True)
    for name in ('v2_width_static.py', 'bounded_state.py', 's2_preview_core_width.py'):
    ...
    try:
        ...
        while time.monotonic()-begin < 10:
            ...
            row = dict(frame=len(rows)+1, elapsed_s=elapsed, sensor_timestamp=sensor,
                ...
                vision_ms=(done-acquired)*1000, control_status=result['control_status'], target_source=result['target_source'],
        ...
        for number, frame, result in snapshots:
            ...
            cv2.putText(view,f"{result['control_status']} {result['target_source']} PWM={result['steering_pwm']}",(8,22),cv2.FONT_HERSHEY_SIMPLEX,.5,(0,255,0),1)
