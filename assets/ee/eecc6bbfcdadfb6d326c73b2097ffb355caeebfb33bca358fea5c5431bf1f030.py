...
def main():
    ...
    out = Path('straight_fixed365_results')/datetime.now().strftime('%Y%m%d_%H%M%S_%f')
    out.mkdir(parents=True)
    for name in ('v2_fixed365_test.py', 'bounded_state.py', 's2_preview_core_calibrated.py'):
    ...
    (out/'settings.json').write_text(json.dumps(dict(drive=args.drive, duration=2,
        ...
        servo_center=365, fixed_steering=365, servo_range=[290,440], initial_ready_frames=5), indent=2))
    camera = pwm = output = None
    rows = []; snapshots = []; error = None
    try:
        if args.drive:
            confirm('固定365直行对照：驱动电机电池先断开。小车放在直道中央，车头沿车道，前方清空并准备断电。确认后输入YES：')
        ...
        if args.drive:
            confirm('已发送停止370。接通电机电池并等待ESC就绪。输入YES后倒数3秒，目标连续有效才前进，固定转向365，最多2秒。不要遮挡镜头；准备随时断电：')
        ...
        while time.monotonic()-begin < 10:
            ...
            last_sensor = sensor
            vision_command = result.get('steering_pwm')
            command = 365
            ...
            row = dict(frame=len(rows)+1, elapsed_s=elapsed, sensor_timestamp=sensor,
                ...
                vision_steering_pwm=vision_command,
        ...
        for number, frame, result in snapshots:
            ...
            cv2.putText(view,f"{result['control_status']} fixed PWM=365",(8,22),cv2.FONT_HERSHEY_SIMPLEX,.5,(0,255,0),1)
