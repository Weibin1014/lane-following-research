...
class Tracker:
    ...
    def analyze(self,frame,t):
        ...
        heights=np.array(sorted(shared))
        shifts=np.array([white[y]-ow[y] for y in heights])
        # Fit height-dependent motion, retaining displacement and residual limits.
        motion=np.polyfit(heights,shifts,1)
        prediction=np.polyval(motion,heights)
        if max(abs(prediction))>4 or max(abs(shifts-prediction))>2:return r
