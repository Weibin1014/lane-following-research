...
from bounded_state_local_8s import Gate, self_test, DURATION_SECONDS
...
def main():
    ...
    out = Path('transition_local_8s_results')/datetime.now().strftime('%Y%m%d_%H%M%S_%f')
    out.mkdir(parents=True)
    for name in ('v2_local_transition_8s.py', 'bounded_state_local_8s.py', 's2_preview_core_local.py'):
    ...
    try:
        if args.drive:
            confirm('直道进入左弯连续测试：驱动电机电池先断开。小车放在此前验证的左弯入口前一小段直道中央，车头沿当地车道方向，前方清空并准备断电。确认后输入YES：')
