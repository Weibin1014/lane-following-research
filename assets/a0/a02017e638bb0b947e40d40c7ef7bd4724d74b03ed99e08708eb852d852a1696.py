...
import numpy as np
import cv2
_paths_original_analyze = analyze

def analyze(frame):
    original=_paths_original_analyze(frame)
    ...
    for _,track in options:
        ...
        points.sort()
        result=control(points,w,'selected')
    ...
    for x in valid[1:]:
        ...
        ax,_=target(x['model_points'],shared_far)
        bx,_=target(valid[0]['model_points'],shared_far)
