...
CORE_HASH = '23721951d76ae6a29b7e0692dbea43977c4eb9d928f0f7744f00af1cd01bd4b1'
...
def main():
    ...
    if args.self_test:
        self_test()
        return
    core = Path(__file__).with_name('s2_preview_core_window.py')
    if hashlib.sha256(core.read_bytes()).hexdigest() != CORE_HASH:
        raise RuntimeError('V2核心哈希不同，停止测试')
    import cv2
    from picamera2 import Picamera2
    from s2_preview_core_window import analyze
    out = Path('curve_window_static_results')/datetime.now().strftime('%Y%m%d_%H%M%S_%f')
    out.mkdir(parents=True)
    for name in ('v2_window_static.py', 'bounded_state.py', 's2_preview_core_window.py'):
