"""Default: camera-only dry run. --drive: bounded 5-second straight-track test."""
import argparse
import csv
import hashlib
import json
...
def main():
    ...
    out = Path('straight_thin_5s_results')/datetime.now().strftime('%Y%m%d_%H%M%S_%f')
    out.mkdir(parents=True)
    for name in ('v2_thin_straight_5s.py', 'bounded_state_5s.py', 's2_preview_core_thin.py'):
    ...
    try:
        if args.drive:
            confirm('直道落地测试：驱动电机电池先断开。小车放在直道中央，车头沿直道方向，前方直道长度应足够本次5秒行驶，前方清空并准备断电。确认后输入YES：')
