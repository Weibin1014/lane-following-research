"""Default: camera-only dry run. --drive: bounded 5-second curve-exit test."""
import argparse
import csv
import hashlib
import json
...
def main():
    ...
    out = Path('exit_thin_5s_results')/datetime.now().strftime('%Y%m%d_%H%M%S_%f')
    out.mkdir(parents=True)
    for name in ('v2_thin_exit_5s.py', 'bounded_state_5s.py', 's2_preview_core_thin.py'):
    ...
    try:
        if args.drive:
            confirm('左弯出口至直道测试：驱动电机电池先断开。小车保持已检查的弯道末端位置，车头沿当地车道方向，前方清空并准备断电。确认后输入YES：')
