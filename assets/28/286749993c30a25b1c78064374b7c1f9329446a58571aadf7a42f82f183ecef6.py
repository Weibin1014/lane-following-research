...
CORE_HASH = 'd115aa995bbdd671eb9a35d0ee7d9232cb5da6cedab3b21bcc701a7c08bce641'
...
def main():
    ...
    if args.self_test:
        self_test()
        return
    core = Path(__file__).with_name('s2_preview_core_center365.py')
    if hashlib.sha256(core.read_bytes()).hexdigest() != CORE_HASH:
        raise RuntimeError('V2核心哈希不同，停止测试')
    import cv2
    from picamera2 import Picamera2
    from s2_preview_core_center365 import analyze
    out = Path('straight_center365_results')/datetime.now().strftime('%Y%m%d_%H%M%S_%f')
    out.mkdir(parents=True)
    for name in ('v2_center365_test.py', 'bounded_state.py', 's2_preview_core_center365.py'):
