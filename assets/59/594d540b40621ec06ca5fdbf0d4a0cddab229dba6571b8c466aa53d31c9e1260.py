...
CORE_HASH = '42c941bc3d5eeddb6058d7d478c6f3afe88169cf90a68ca5ab6f52b6e51c31d4'
...
def main():
    ...
    if args.self_test:
        self_test()
        return
    core = Path(__file__).with_name('s2_preview_core_paths.py')
    if hashlib.sha256(core.read_bytes()).hexdigest() != CORE_HASH:
        raise RuntimeError('V2核心哈希不同，停止测试')
    import cv2
    from picamera2 import Picamera2
    from s2_preview_core_paths import analyze
    out = Path('curve_paths_static_results')/datetime.now().strftime('%Y%m%d_%H%M%S_%f')
    out.mkdir(parents=True)
    for name in ('v2_paths_static.py', 'bounded_state.py', 's2_preview_core_paths.py'):
