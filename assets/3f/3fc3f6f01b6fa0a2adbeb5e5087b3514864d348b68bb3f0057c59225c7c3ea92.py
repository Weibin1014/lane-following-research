"""Default: camera-only dry run. --bench: raised-wheel motor test only."""
import argparse
import csv
import hashlib
import json
...
from bounded_state import Gate, self_test

CORE_HASH = 'b25fe0fbacf299d77ee8317638d4c0d70d645cf835565326ff4307f57e509c4a'
...
class Output:
    ...
    def update(self, ready, steering, fresh):
        with self.lock:
            ...
            steering = steering if ready and fresh and not self.gate.reason else 350
        ...
        while not self.closed.wait(.01):
            with self.lock:
                if self.gate.started is None or self.gate.reason:
                    continue
                now = time.monotonic()
                if now-self.gate.started >= 2:
                    self.gate.stop('time_limit')
                elif now-self.last > .25:
                    self.gate.stop('watchdog')
                if self.gate.reason:
                    try: self.write(370, 350)
...
def main():
    ...
    if args.self_test:
        self_test()
        return
    core = Path(__file__).with_name('s2_preview_core_v2.py')
    ...
    out = Path('bounded_v2_results')/datetime.now().strftime('%Y%m%d_%H%M%S_%f')
    out.mkdir(parents=True)
    for name in ('v2_bounded_test.py', 'bounded_state.py', 's2_preview_core_v2.py'):
        shutil.copy2(Path(__file__).with_name(name), out/name)
    (out/'settings.json').write_text(json.dumps(dict(bench=args.bench, duration=2,
        watchdog_seconds=.25, fresh_limit_seconds=.2, throttle=400, stop=370,
        servo_center=350, servo_range=[330,390], initial_ready_frames=5), indent=2))
    camera = pwm = output = None
    rows = []; snapshots = []; error = None
    try:
        if args.bench:
            confirm('仅架空测试：驱动轮离地，驱动电机电池先断开，舵机供电。确认后输入YES：')
        ...
        if args.bench:
            confirm('已发送停止370。保持车轮架空，接通电机电池并等待ESC就绪。输入YES后最多运行2秒；准备遮挡镜头：')
            for n in (3,2,1): print(n, flush=True); time.sleep(1)
        else:
            print('DRY RUN：不导入PCA9685、不写PWM。模拟一次2秒控制；请保持电机电池断开。', flush=True)
        output = Output(pwm)
        ...
        while time.monotonic()-begin < 10:
            ...
            ready = (result.get('control_status') == 'ready' and
                     command is not None and math.isfinite(command) and 330 <= command <= 390)
            throttle, steering, reason = output.update(ready, int(command) if ready else 350, fresh)
            ...
            row = dict(frame=len(rows)+1, elapsed_s=elapsed, sensor_timestamp=sensor,
                ...
                requested_throttle=throttle, requested_steering=steering, pwm_enabled=args.bench, stop_reason=reason)
            rows.append(row)
        ...
        try:
            if output is not None: output.stop()
                ...
                pwm.set_pwm(0,0,370); pwm.set_pwm(1,0,350)
        except Exception as exc:
        ...
        summary=dict(frames=len(rows), stop_reason=output.gate.reason if output else 'setup_failure',
            hardware_pwm=args.bench, error=error, watchdog_error=output.fault if output else None,
