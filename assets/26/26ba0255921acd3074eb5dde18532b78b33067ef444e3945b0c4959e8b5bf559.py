...
def send(mode, throttle, recording=False):
    data = json.dumps({
        "angle": 0.0, "throttle": throttle,
        "drive_mode": mode, "recording": recording
    }).encode()
...
import sys
if '--check-recording' in sys.argv:
    try:
        send('local_angle', 0.0, recording=True)
        print('Zero throttle CNN recording test: 5 seconds.', flush=True)
        time.sleep(5)
    finally:
        send('user', 0.0, recording=False)
    print('Recording test ended; zero-throttle request acknowledged.', flush=True)
    raise SystemExit(0)

out = Path("pixel_evaluation_20260912/cnn_control_logs") / datetime.now().strftime("%Y%m%d_%H%M%S_%f")
...
result = {
    ...
    "recording_requested_during_drive": True,
...
try:
    ...
    send("local_angle", 0.375, recording=True)
