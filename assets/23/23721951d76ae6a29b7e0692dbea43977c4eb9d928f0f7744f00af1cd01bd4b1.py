...
_SCOPE['ratios'] = np.arange(0.9, 0.549, -0.05)
_SCOPE['scope'] = _SCOPE
...
def analyze(frame):
    global raw_white
    top=max(0, int(frame.shape[0]*.55)-6)
    ...
    finally:_SCOPE['road_white_segments']=original
...
def analyze(frame):
    ...
    try:return _edge_analyze(frame)
    finally:_SCOPE['build_color_masks']=original
