"""Default: camera-only dry run. --drive: bounded straight-track test."""
import argparse
import csv
import hashlib
import json
...
CORE_HASH = '7ca710bfb25fdbfa527dda6998eeac2739bc0ef1496d284e6ebcea5223c17e2c'
...
def main():
    ...
    if args.self_test:
        self_test()
        return
    core = Path(__file__).with_name('s2_preview_core_calibrated.py')
    if hashlib.sha256(core.read_bytes()).hexdigest() != CORE_HASH:
        raise RuntimeError('V2核心哈希不同，停止测试')
    import cv2
    from picamera2 import Picamera2
    from s2_preview_core_calibrated import analyze
    out = Path('straight_calibrated_results')/datetime.now().strftime('%Y%m%d_%H%M%S_%f')
    out.mkdir(parents=True)
    for name in ('v2_calibrated_test.py', 'bounded_state.py', 's2_preview_core_calibrated.py'):
        shutil.copy2(Path(__file__).with_name(name), out/name)
    (out/'settings.json').write_text(json.dumps(dict(drive=args.drive, duration=2,
        watchdog_seconds=.25, fresh_limit_seconds=.2, throttle=400, stop=370,
        servo_center=350, servo_range=[290,440], initial_ready_frames=5), indent=2))
    camera = pwm = output = None
    rows = []; snapshots = []; error = None
    try:
        if args.drive:
            confirm('直道落地测试：驱动电机电池先断开。小车放在直道中央，车头沿车道，前方清空并准备断电。确认后输入YES：')
        ...
        if args.drive:
            confirm('已发送停止370。接通电机电池并等待ESC就绪。输入YES后倒数3秒，目标连续有效才前进，最多2秒。不要遮挡镜头；准备随时断电：')
