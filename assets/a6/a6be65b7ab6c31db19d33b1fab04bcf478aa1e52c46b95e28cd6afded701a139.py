"""Default: camera-only dry run. --drive: bounded 5-second left-curve test."""
import argparse
import csv
import hashlib
import json
...
from bounded_state_5s import Gate, self_test, DURATION_SECONDS
...
def main():
    ...
    out = Path('second_curve_local_5s_results')/datetime.now().strftime('%Y%m%d_%H%M%S_%f')
    out.mkdir(parents=True)
    for name in ('v2_local_second_curve_5s.py', 'bounded_state_5s.py', 's2_preview_core_local.py'):
    ...
    try:
        if args.drive:
            confirm('另一端左弯落地测试：驱动电机电池先断开。小车保持已检查的另一端左弯入口位置，车头沿当地车道方向，前方清空并准备断电。确认后输入YES：')
        ...
        if args.drive:
            confirm('已发送停止370。接通电机电池并等待ESC就绪。输入YES后倒数3秒，目标连续有效才前进，最多5秒。不要遮挡镜头；准备随时断电：')
            for n in (3,2,1): print(n, flush=True); time.sleep(1)
        else:
            print('DRY RUN：不导入PCA9685、不写PWM。模拟一次5秒控制；请保持电机电池断开。', flush=True)
        output = Output(pwm)
