"""Default: camera-only dry run. --drive: bounded 30-second continuous test."""
import argparse
import csv
import hashlib
import json
...
from bounded_state_local_30s import Gate, self_test, DURATION_SECONDS

CORE_HASH = '95d59d65510ef4596948c277b16e3def86fe404a8aae71b4b4402715b7c1efa8'
...
def main():
    ...
    if args.self_test:
        self_test()
        return
    core = Path(__file__).with_name('s2_preview_core_local.py')
    ...
    out = Path('/home/student/projects/mycar/pixel_evaluation_20260912/opencv_runs')/datetime.now().strftime('%Y%m%d_%H%M%S_%f')
    out.mkdir(parents=True)
    for name in ('v2_local_continuous_30s.py', 'bounded_state_local_30s.py', 's2_preview_core_local.py'):
    ...
    try:
        if args.drive:
            confirm('30秒连续路段测试：驱动电机电池先断开。小车放在本轮与CNN一致的约定起点处、车道中央，车头沿当地车道方向，前方清空并准备断电。确认后输入YES：')
        ...
        if args.drive:
            confirm('已发送停止370。接通电机电池并等待ESC就绪。输入YES后倒数3秒，目标连续有效才前进，最多30秒。不要遮挡镜头；准备随时断电：')
            for n in (3,2,1): print(n, flush=True); time.sleep(1)
        else:
            print('DRY RUN：不导入PCA9685、不写PWM。模拟一次30秒控制；请保持电机电池断开。', flush=True)
        output = Output(pwm)
        begin = time.monotonic(); last_sensor = None; last_save = -1
        while time.monotonic()-begin < DURATION_SECONDS + 5:
            ...
            row = dict(frame=len(rows)+1, elapsed_s=elapsed, sensor_timestamp=sensor,
                ...
                vision_ms=(done-acquired)*1000, control_status=result['control_status'], target_source=result.get('target_source', 'paired' if ready else 'unavailable'), near_extrapolation=result.get('near_extrapolation'), near_uncertainty_px=result.get('near_uncertainty_px'),
            ...
            if elapsed-last_save >= .1 or not ready or reason:
                if len(snapshots) < 800: snapshots.append((len(rows), frame, result))
                last_save = elapsed
            if reason: break
        ...
        for number, frame, result in snapshots:
            ...
            cv2.putText(view,f"{result['control_status']} {result.get('target_source', 'paired' if result['control_status']=='ready' else 'unavailable')} PWM={result['steering_pwm']}",(8,22),cv2.FONT_HERSHEY_SIMPLEX,.5,(0,255,0),1)
