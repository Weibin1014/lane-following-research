...
import cv2
from s2_preview_core import analyze
...
def main():
    ...
    out = Path('preview_steering_results') / datetime.now().strftime('%Y%m%d_%H%M%S_%f')
    out.mkdir(parents=True)
    for name in ('s2_preview_core.py', 'realtime_preview_steering.py'):
    ...
    try:
        if args.steering:
            print('本程序仅做静态舵机验证。驱动电机电池必须一直断开，舵机保持供电。',flush=True)
        ...
        begin=time.perf_counter(); next_save=0; next_log=0
        fields=['frame','elapsed_s','capture_wait_ms','vision_and_pwm_ms','track_status','control_status',
            'near_status','far_status','near_offset','far_y','far_x','preview_error','steering_pwm','pwm_sent']
            ...
            while time.perf_counter()-begin < 30:
                ...
                if elapsed>=next_save:
                    ...
                    for suffix,img in [('raw',frame),('debug',view)]:
                        if not cv2.imwrite(str(out/f'{frames:06d}_{suffix}.jpg'),img):raise RuntimeError('图片保存失败')
                if elapsed>=next_log:
